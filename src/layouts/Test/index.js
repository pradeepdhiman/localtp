...
const Test = () => {
  ...
  const startCountdown = async () => {
    if (!Object.keys(assessmentItem).length) {
    ...
    try {
      ...
      if (res?.data?.success) {
        ...
        MySwal.fire({
          icon: 'error',
          title: 'Error!',
          text: res?.data?.errors[0] || "Something went wrong.",
          confirmButtonText: 'Ok',
          showCancelButton: true,
        }).then(result => {
          if (result.isConfirmed) {
        ...
        <Card elevation={3}>
              ...
              <Grid item>
                <FormControl>
                  <RadioGroup value={answer} onChange={answerChangeHandler}>
                    {question?.data && Object.entries(question.data).map(([key, value]) => {
          ...
          <SoftBox py={3} px={3} sx={{ display: "flex", justifyContent: "space-between", alignItem: "center" }}>
            ...
            {answeerlist?.length + 1 === parseInt(assessResp?.data?.numberofQuestions) ?
              <SoftButton disabled={!answer} onClick={() => nextHandler("submit")} color="info" variant="gradient">{submitLoading ? "Loading..." : "Submit Test"}</SoftButton> :
              <SoftButton disabled={!answer} onClick={nextHandler} color="info" variant="gradient">{submitLoading ? "Loading..." : "Next"}</SoftButton>
            }
